    ...
    <section className="py-16 bg-gray-50 dark:bg-slate-900/60" id="skills">
      <div className="max-w-6xl mx-auto px-6 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="bg-white shadow-lg rounded-2xl p-6 dark:bg-slate-800 dark:shadow-slate-950/40">
            <h3 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <span className="text-purple-600">🗄️</span> Database
            </h3>
          ...
          <div className="bg-white shadow-lg rounded-2xl p-6 dark:bg-slate-800 dark:shadow-slate-950/40">
            <h3 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <span className="text-purple-600">🗄️</span> Tools
            </h3>
            {skillsData?.skills?.tools && skillsData.skills.tools.map((skill: any) => (
              <div key={skill.name} className={`mb-4 ${skill.visible ? '' : 'hidden'}`}>
                <div className="flex justify-between mb-1">
                  <span className="text-gray-700 dark:text-slate-200">{skill.name}</span>
                  <span className="text-gray-600 text-sm dark:text-slate-400">{skill.level}%</span>
          ...
          <div className="bg-white shadow-lg rounded-2xl p-6 dark:bg-slate-800 dark:shadow-slate-950/40">
            <h3 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <span className="text-purple-600">🗄️</span> Concepts
            </h3>
            {skillsData?.skills?.concepts && skillsData.skills.concepts.map((skill: any) => (
              <div key={skill.name} className={`mb-4 ${skill.visible ? '' : 'hidden'}`}>
                <div className="flex justify-between mb-1">
                  <span className="text-gray-700 dark:text-slate-200">{skill.name}</span>
                  <span className="text-gray-600 text-sm dark:text-slate-400">{skill.level}%</span>
          ...
          <div className="bg-white shadow-lg rounded-2xl p-6 dark:bg-slate-800 dark:shadow-slate-950/40">
            <h3 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <span className="text-purple-600">🗄️</span> Cloud
            </h3>
            {skillsData?.skills?.cloud && skillsData.skills.cloud.map((skill: any) => (
              <div key={skill.name} className={`mb-4 ${skill.visible ? '' : 'hidden'}`}>
                <div className="flex justify-between mb-1">
                  <span className="text-gray-700 dark:text-slate-200">{skill.name}</span>
                  <span className="text-gray-600 text-sm dark:text-slate-400">{skill.level}%</span>
        ...
        <div className="mt-12">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {skillsData?.skills?.others && skillsData.skills.others.map((tech: any) => (
              <span
                key={tech}
